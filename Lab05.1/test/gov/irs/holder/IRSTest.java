...
public class IRSTest {
  ...
  public void testNormalClientUsage() {
    IRS irs = IRS.getInstance(); //NOTE: You know getInstance method is a static method because you can only invoke this on a class. (You can tell IRS is a class by capitalization)
    //NOTE this is also an excample of a singleton
   ...
   */
  @Test
  public void testSingleton() { //NOTE  you can test if signleton working by checking if two instances point at same place in memory. (The singleton is only creating additional references to the already created object.)
