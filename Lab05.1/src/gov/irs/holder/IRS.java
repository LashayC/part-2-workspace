...
public class IRS {
  ...
  public static IRS getInstance() {
    return IRSHolder.instance; //NOTE once this is initialized, if its invoked again it'll just return a reference to the object.
  }
  ...
  //NOTE: The initialization, on demand holder idiom. Involves a private static holder class that holds that holds on to that singleton instance of the enclosing class.
  //NOTE: The reason the enclosing class can see the below private class is bc its nested. Still can't be seen outside IRS class tho.
  private static class IRSHolder {
    private static IRS instance = new IRS(); //NOTE you'd add final here to be sure that it can only be given one value. This would be created during class initialization.
